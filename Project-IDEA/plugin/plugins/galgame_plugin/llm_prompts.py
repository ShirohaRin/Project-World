...
_USER_PROMPT_PREFIXES = {
    ...
    "summarize_scene": (
        ...
        "Requirements:\n"
        "0. This output is a cumulative memory artifact only. Do not write a "
        "cat-facing reply, dialogue, or instruction to react to an earlier line.\n"
        "1. summary: 1-3 sentences giving a cumulative summary of the current scene. "
        "Preserve the established scene progression and integrate new facts; do not "
        "replace the cumulative summary with a delta-only summary.\n"
        "2. key_points.type must be one of: plot / emotion / decision / reveal / objective.\n"
        "3. key_points must only reference facts supported by context. When "
        "new_stable_lines or new_choices are present, prioritize those newly added "
        "facts in key_points; older content is continuity background only.\n"
        "4. stable_lines are confirmed cumulative plot facts and should be the primary "
        "basis for summary. new_stable_lines, when present, is the newly added subset.\n"
        "5. observed_lines are OCR candidates and should only be treated as "
        "\"possibly recent lines\", never as confirmed facts.\n"
        "6. recent_choices are cumulative player-confirmed selections. new_choices, "
        "when present, is the newly added subset. A new choice with choice_state "
        "\"selected\" is player-confirmed; choice_state \"visible\" is only currently "
        "available and must never be described as selected. Prioritize decision or "
        "objective key_points for newly confirmed selections.\n"
        ...
        "should not be copied verbatim. Use it and older stable content only to maintain "
        "continuity around the newly added facts.\n"
        "10. For backward compatibility, if the new_* fields are absent, treat the "
        "available stable_lines and recent_choices as the initial newly available "
        "content. If new_* fields are present but empty, do not manufacture a new event.\n"
        "11. Output must match this JSON structure:\n"
    ),
    "suggest_choice": (
