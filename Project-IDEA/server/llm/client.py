...
class LLMClient:
    ...
    async def chat(
        ...
        model_id = self.get_model_id(provider, model_name)

        url = f"{self.get_base_url(provider)}/chat/completions"
        api_key = self.api_keys.get(provider, "")
        if not api_key:
            return self._fallback_response(messages, "未配置 LLM API Key。请设置环境变量 DOUBAO_API_KEY 或 GLM_API_KEY")
    ...
    async def chat_stream(
        ...
        if not api_key:
            yield {"type": "text", "content": "⚠️ 未配置 LLM API Key。请设置环境变量 DOUBAO_API_KEY 或 GLM_API_KEY\n\n"}
            # Fallback: return structured response
            fallback = self._fallback_response(messages, "无 API Key")
