...
function TaskSidebar({ workspace, workspaceName, onChooseWorkspace, onCreateTask, onOpenTask, onSwitchToIde, showWorkspace = true, newTask, serviceState }: { workspace: string; workspaceName: string; onChooseWorkspace: () => void; onCreateTask: () => void; onOpenTask: () => void; onSwitchToIde?: () => void; showWorkspace?: boolean; newTask: boolean; serviceState: ServiceState }) {
  ...
  return <aside className="chat-sidebar"><button className="new-chat-button" onClick={onCreateTask}><span>＋</span>新建任务</button><div className="chat-sidebar-label">今天</div><button className={newTask ? 'chat-history-item' : 'chat-history-item active'} onClick={onOpenTask}><span className="history-dot" />工作会话</button>{showWorkspace ? <><div className="chat-sidebar-label">工作区</div><button className="workspace-button" title={workspace} onClick={onChooseWorkspace}><span className="chevron">⌄</span>{workspaceName}</button></> : null}<div className="chat-sidebar-footer"><span className={`service-status ${serviceState}`}><span />{serviceLabel}</span>{onSwitchToIde ? <button title="切换至 IDE" onClick={onSwitchToIde}>⌘</button> : null}</div></aside>
}
...
export default function App() {
  ...
  async function sendChatMessage() {
    ...
    setIsSending(true)
    try {
      const result = await window.ideaDesktop?.sendChat({ agentId: 'idea', message: content, conversationId })
...
function ChatComposer({ input, setInput, onSend, selectedAgent, serviceState, isSending }: { input: string; setInput: (value: string) => void; onSend: () => void; selectedAgent: string; serviceState: ServiceState; isSending: boolean }) {
  const note = serviceState === 'online' ? '在线模式会调用当前账户获授权的智能体，并由服务端执行受控任务。' : serviceState === 'checking' ? '正在连接 IDEA 服务。' : serviceState === 'unauthorized' ? '登录会话无效，请在设置中重新登录。' : serviceState === 'error' ? '服务暂不可用，本地文件能力不受影响。' : '请在设置中配置 IDEA 服务地址并使用邮箱登录。'
